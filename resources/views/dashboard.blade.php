    ...
    <div class="py-12 bg-gray-100">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl shadow">
                <h3 class="font-bold mb-2">Critical Assets (≤ 10 items)</h3>
                        ...
                        <tbody class="text-gray-800 bg-white">
                            @forelse ($criticalAssets as $aset)
                                <tr class="hover:bg-gray-100">
                                    <td class="p-2 border-t border-r">{{ $aset->kategori }}</td>
                                    <td class="p-2 border-t border-r">{{ $aset->merk }}</td>
                                    <td class="p-2 border-t border-r">{{ $aset->spesifikasi }}</td>
                                    <td class="p-2 border-t text-red-600 font-bold">{{ $aset->jumlah }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="p-2 text-center text-gray-500">Tidak ada aset kritis</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                    ...
                    <table class="min-w-full text-center">
                        ...
                        <tbody class="text-gray-800 bg-white">
                            @forelse ($recentActivities as $item)
                                <tr class="hover:bg-gray-100">
                                    <td class="p-2 border-t border-r">{{ \Carbon\Carbon::parse($item->tanggal)->format('d M Y') }}</td>
                                    <td class="p-2 border-t border-r">{{ $item->kategori }}</td>
                                    <td class="p-2 border-t border-r">{{ ucfirst($item->status) }}</td>
                                    <td class="p-2 border-t">{{ $item->progress }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="p-2 text-center text-gray-500">Belum ada aktivitas terbaru</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
