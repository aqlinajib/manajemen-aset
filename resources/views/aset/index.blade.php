    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="p-3">
                                <div class="relative inline-block text-left">
                                    <button @click="$refs.dropdown{{ $aset->id }}.classList.toggle('hidden')"
                                    class="bg-gray-100 hover:bg-gray-200 text-sm px-3 py-1 rounded">
                                        Aksi ▾
                                    </button>
                                    <div x-ref="dropdown{{ $aset->id }}" class="absolute right-0 mt-2 w-28 bg-white border border-gray-200 shadow-md rounded z-10 hidden">
                                        ...
                                        <a href="{{ route('aset.edit', $aset->id) }}"
                                            class="block px-4 py-2 text-sm text-yellow-600 hover:bg-yellow-50">Edit</a>
        ...
        <div x-cloak x-show="showModal" x-transition
            ...
            <div class="bg-white p-6 rounded shadow-md max-w-sm w-full">
                ...
                <p class="text-gray-700 mb-6">Apakah Anda yakin ingin menghapus aset ini?</p>
                <div class="flex justify-end space-x-3">
                    <button @click="showModal = false"
                            class="px-4 py-2 bg-gray-300 rounded hover:bg-gray-400">Batal</button>
                    <form :action="'/aset/' + selectedId" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
